...
int32_t loopback_tcps(uint8_t sn, uint8_t *buf, uint16_t port)
{
   ...
   case SOCK_CLOSE_WAIT:
      ...
      // printf("%d:CloseWait\r\n",sn);
#endif
      if ((ret = WIZ_disconnect(sn)) != SOCK_OK)
         return ret;
#ifdef _LOOPBACK_DEBUG_
...
int32_t loopback_tcpc(uint8_t sn, uint8_t *buf, uint8_t *destip, uint16_t destport)
{
   ...
   case SOCK_ESTABLISHED:
      ...
      if ((size = getSn_RX_RSR(sn)) > 0) // Sn_RX_RSR: Socket n Received Size Register, Receiving data length
      {
         ...
         ret = WIZ_recv(sn, buf, size); // Data Receive process (H/W Rx socket buffer -> User's buffer)
         printf("%s", buf);
         ...
         while (size != sentsize)
         {
            ret = WIZ_send(sn, buf + sentsize, size - sentsize); // Data send process (User's buffer -> Destination through H/W Tx socket buffer)
...
#endif
      if ((ret = WIZ_connect(sn, destip, destport)) != SOCK_OK)
...
int32_t loopback_udps(uint8_t sn, uint8_t *buf, uint16_t port)
{
   ...
   {
   case SOCK_UDP:
            ret = WIZ_sendto(sn, buf, 2048, destip, destport);
